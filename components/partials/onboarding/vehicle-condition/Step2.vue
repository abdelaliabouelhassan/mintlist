<template>
    <div class=" w-full flex flex-col items-start space-y-2">
        ...
        <div class=" w-full flex flex-col items-start space-y-4 pt-4 relative">
            <div  :class="{'bg-[#36bbbc]':option.checked,'bg-[#fafafa]':!option.checked}" class=" w-full  rounded-[4px]  cursor-pointer"  v-for="(option,index,key) in store.$state.form.vehicle_condition.conditions" :key="key">
...
<script setup>
    ...
    const isDisabled1 = computed(()=>{
         return store.$state.form.vehicle_condition.conditions.some((item)=>{
   ...
   watch(()=>store.$state.form.vehicle_condition.conditions,()=>{
    ...
    const Select = (option) => {
       if(isDisabled1.value && option.val != 'no_issues' || isDisabled2.value && option.val == 'no_issues'){
        ...
        store.$state.form.vehicle_condition.conditions.forEach((item)=>{
