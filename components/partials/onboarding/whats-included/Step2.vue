<template>
    <div class=" w-full flex flex-col items-start space-y-2">
        ...
        <p class=" text-white text-lg font-bold font-pnova">
           Do you have all your vehicle's service records, warranty paperwork, and owner's manual?
        </p>
        <p class=" text-[#fafafa] text-sm font-normal font-pnova">
            Please select one
        </p>
        <div class=" w-full flex flex-col items-start space-y-4 pt-4 relative">
            <div :class="{'bg-[#36bbbc]':store.$state.form.whats_included.service_record === option.value,'bg-[#fafafa]':store.$state.form.whats_included.service_record != option.value}" class=" w-full  rounded-[4px] p-4 py-4 cursor-pointer" @click="Select(option)" v-for="(option,index,key) in options" :key="key">
                <div class=" w-full flex items-center ">
                    <div  class="radioButton isTickButton mt-1.5">
                        <label class="label">
                            <input data-v-7dc0630a="" :checked="store.$state.form.whats_included.service_record === option.value"  name="service_record" id="radio-0" type="radio" class="">
                        </label>
                    </div>
                    <div class=" flex flex-col items-start space-y-px " :class="{'text-white':store.$state.form.whats_included.service_record === option.value,'text-black':store.$state.form.whats_included.service_record != option.value}">
...
<script setup>
    ...
    const Select = (option) => {
        store.$state.form.whats_included.service_record = option.value
     ...
     const isFormNotEmpty = computed(()=>{
        return store.$state.form.whats_included.service_record !== ''
     })
</script>